...
//-->
	function parseValue(text)	{
		...
		function move(cmbsource,cmbtarget)
		{
			for(var i=cmbsource.length-1;i>=0;i--)
			{
				var branch = cmbsource.options[i];
				if (branch.selected)
				{
					cmbtarget.options[cmbtarget.options.length] = new Option(branch.text, branch.value);
					cmbsource.options[i]=null;
				}
			}
			
		}
		function moveAll(cmbsource,cmbtarget)
		{
			for(var i=0;i<cmbsource.length;i++)
			{
				var branch = cmbsource.options[i];
				cmbtarget.options[cmbtarget.options.length] = new Option(branch.text, branch.value);
			}
